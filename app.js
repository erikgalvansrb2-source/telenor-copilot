...
(() => {
  ...
  fetch('api-key 1.txt').then(r=>r.text()).then(k=>loadMaps(k.trim())).catch(()=>alert('Put your Google Maps JS API key into "api-key 1.txt"'));
  function loadMaps(key){ const s=document.createElement('script'); s.src=`https://maps.googleapis.com/maps/api/js?key=${encodeURIComponent(key)}&libraries=geometry,drawing&callback=initMap`; s.async=true; s.defer=true; document.head.appendChild(s); }

  window.initMap = function(){
    $('kmValue').textContent = `${LTE_MIN_DISTANCE_KM} km`;
    map = new google.maps.Map($('map'), {center:{lat:0,lng:0}, zoom:2, mapTypeId:'hybrid', streetViewControl:false});
  ...
  function onPos(pos){ const {latitude:lat, longitude:lng, accuracy} = pos.coords; const p=new google.maps.LatLng(lat,lng); if(!userMarker){ userMarker=new google.maps.Marker({map, position:p, title:'Your position'}); map.setCenter(p); map.setZoom(8);} else { userMarker.setPosition(p);} if(!accuracyCircle){ accuracyCircle=new google.maps.Circle({map, center:p, radius:accuracy||50, strokeColor:'#49a0ff', strokeOpacity:.6, strokeWeight:1, fillColor:'#49a0ff', fillOpacity:.12}); } else { accuracyCircle.setCenter(p); accuracyCircle.setRadius(accuracy||50);} $('accuracy').textContent = accuracy? `${accuracy.toFixed(0)} m` : '–'; evaluate(); }
