...
// Initialize after dynamic loader is available
(async function init(){
  $('kmValue').textContent = `${LTE_MIN_DISTANCE_KM} km`;

  // Load Maps libraries on demand
...
async function onPos(pos){ const {latitude:lat, longitude:lng, accuracy} = pos.coords; const p=new google.maps.LatLng(lat,lng);
  if(!userMarker){
    const { AdvancedMarkerElement } = await google.maps.importLibrary('marker');
    userMarker=new AdvancedMarkerElement({ map, position:p, title:'Your position' });
    map.setCenter(p); map.setZoom(8);
  } else {
